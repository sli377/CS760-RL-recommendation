...
# Custom evaluation metric for Yelp dataset
def calculate_metrics_for_yelp(sorted_list, topk, true_items, rewards,
                               total_reward, hit, ndcg, 
                               q_values_neg, q_values_mb,
...
def calculate_hit_wqestimates_improved(sorted_list, topk, true_items, rewards, r_click,
                                 total_reward, hit_click, ndcg_click,
                                 hit_purchase, ndcg_purchase, q_values_neg,
                                 q_values_mb, total_qestimates_neg, total_qestimates_mb):
    """
    改进版的calculate_hit_wqestimates，使用 <= 而不是 == 来区分普通评分和高评分。
    这样可以更准确地分类评分数据。
    """
    for i in range(len(topk)):
        rec_list = sorted_list[:, -topk[i]:]
        for j in range(len(true_items)):
            if true_items[j] in rec_list[j]:
                rank = topk[i] - np.argwhere(rec_list[j] == true_items[j])
                total_reward[i] += rewards[j]
                if rewards[j] <= r_click and rewards[j] > 0:  # 使用 <= 替代 ==
                    hit_click[i] += 1.0
                    ndcg_click[i] += 1.0 / np.log2(rank + 1)
                else:
                    total_qestimates_neg[i] += q_values_neg[j]
